...
### Conventional mail-pool compatibility boundary
...
| Distribution/family | Actual mail pool | Other path | Verified owner and mode |
| --- | --- | --- | --- |
...
This compatibility covers only the traditional single-file mbox at `/var/mail/<username>` or `/var/spool/mail/<username>`. The tool does not traverse Maildir or treat aaPanel's `/www/vmail` as a mail pool. A complete revoke still removes the whole tool-managed Home under its separate Home-safety rules.
