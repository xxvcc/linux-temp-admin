...
git -c user.name='XXV.CC' \
    ...
    tag -s v2.9.2 "$RELEASE_COMMIT" -m 'linux-temp-admin v2.9.2'
git -c gpg.format=openpgp -c gpg.program=/usr/bin/gpg \
    verify-tag --raw v2.9.2
...
exec /opt/lta-release-tools/prepare-release.sh \
  v2.9.2 /srv/linux-temp-admin /srv/release-transfer/v2.9.2-prepared
LTA_PREPARE_RELEASE
```
...
/usr/bin/sudo /usr/bin/env -i \
  ...
  /media/in/v2.9.2-prepared /media/out/v2.9.2-signed
```
...
exec /opt/lta-release-tools/publish-release.sh \
  /srv/release-transfer/v2.9.2-signed /srv/linux-temp-admin
LTA_PUBLISH_RELEASE
```
...
```bash
TAG=v2.9.2  # the failed release; verify this value before running
/usr/bin/sudo /usr/bin/env -i \
  HOME=/root PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin LC_ALL=C \
