...
Status reports account identity, UID, expiry, auto-delete task, identity-quarantine deadline, and registry anomalies. `doctor` also reports orphaned sudoers files and sshd exceptions, plus orphaned, missing, or invalid expiry-revoke and quarantine-finalizer tasks; with no account awaiting a schedule, it does not independently prove that the systemd or `at` backend is available.
