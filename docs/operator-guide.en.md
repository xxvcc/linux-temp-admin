...
Without `--port`, the tool reads the effective port from `sshd -T`, then falls back to the static `sshd_config` or OpenSSH's default port 22. If the effective probe fails, the configuration is corrupt, or the sources cannot provide one reliable conclusion, invite fails and requires an explicit `--port`. `doctor` reports both the best-effort value and original diagnostic instead of presenting the fallback as confirmed.
...
With automatic removal disabled, only `linux-temp-admin revoke` deletes the account and `--hours` is ignored.
...
For compatibility with old automatic tasks, a `linux-temp-admin revoke --yes` command without UID/generation arguments cannot prove that its old deletion intent still names the same account if it collides with a concurrent same-name `invite`. The command warns explicitly, deletes no account, and exits successfully so systemd cannot retry the old task against the new generation; a manually issued non-interactive command of the same shape follows the same rule. After the concurrent operation finishes, run `linux-temp-admin doctor` and invoke `linux-temp-admin revoke` again against the current account.

An account reported by `linux-temp-admin doctor` as `legacy-unverified` carries an old fixed identity marker, so same-name/same-UID reuse cannot be excluded. After manual inspection, it can be recovered only by running `linux-temp-admin revoke --user <name> --force` in an interactive terminal and typing the complete username. If the old v2 registry row still has only nine fields and therefore no recorded UID, this manual path is available only while the current account retains the exact fixed marker and has UID 1000 or greater. Before stripping a grant, disabling login, or deleting anything, the command formally migrates the registry to v5, creates `identity-sequence`, and rechecks both the semantic registry record and complete passwd snapshot; a migration or recheck failure leaves those account states untouched. The historical timer's `--yes --force --confirm-force` arguments and every other non-interactive invocation are denied deletion authority for this account class. If an old timer's UID/generation tuple exactly matches the unbound row, it only removes this tool's sudoers/sshd grants and then cancels the old expiry task; it never reads passwd deletion policy, disables or deletes the account, or changes the registry. A grant-removal failure does not actively cancel the task so systemd can retry; a fired `at` or legacy one-shot job still requires manual handling. Cancellation failure also returns nonzero, and the account and row always remain for manual recovery. Low or root UIDs, reserved names, and marker mismatches remain protected. `linux-temp-admin doctor` reports any surviving old task as orphaned, and `linux-temp-admin cleanup-expired --compact` can also cancel it while retaining the live account and registry row for manual handling.
...
After upgrading from v2.9.1 or another older release, manually verify any live pending creation row shown by `status` or `doctor` really came from a failed invite. Selecting that row in the menu automatically enters the same recovery gate as `linux-temp-admin revoke --user <name> --force`, and an interactive terminal must still type the full username. The tool also checks the random pending generation, GECOS, recorded UID (zero or the current UID), managed Home, non-root UID/GID, and nonempty shell. With systemd, a proved pending identity is immediately stripped of access and retains its exact generation in background quarantine. Only the synchronous fallback weakens it to a UID-only deletion-recovery witness. `--yes`, automatic revoke, uninstall bulk removal, piped input, and every identity mismatch refuse the initial recovery authorization.

After deletion authorization, identity checks, and the pre-artifact job/process quiescence pass succeed, the tool persists a deletion-recovery witness before controlled mail/Home cleanup and `userdel`. After the first Home cleanup it checks jobs, processes, and stable identity again immediately before `userdel` (name, UID/GID, Home, and fifth-field generation witness, or the complete snapshot for an old single-marker account), then reconfirms the same passwd identity and repeats UID/GID-bound Home cleanup in case the final quiescence pass recreated it. If account deletion, the post-deletion mail-spool sweep, or task cleanup is interrupted, `status` and `doctor` show the recovery state, a same-name `invite` refuses to overwrite the witness, and `cleanup-expired --compact` does not discard the witness. When the account is absent or still exactly matches the recorded generation, run `linux-temp-admin revoke --user <name>` to resume. For an absent account, every narrow mail sweep confirms before and after that neither the local passwd database nor NSS contains the name. An identifiable automatic task is retained in either state, but only a systemd job retries automatically under its restart policy; `at` and legacy one-shot jobs require a manual retry. Legacy, unregistered, and pending-rollback paths retain only a UID witness. If that account is still live, inspect it and run `linux-temp-admin revoke --user <name> --force` in an interactive terminal, then type the complete username; every non-interactive invocation is refused. The old automatic task for such a live account is treated as orphaned and cancelled, while the registry witness remains for manual recovery.

For an account recorded with `SequentialID`, that bit also proves creation used one reserved number for both the UID and private GID. Before Home/mail cleanup, final deletion requires the same-name private group to remain present with that GID, an explicitly disabled `/etc/group` password (`x`, `*`, or an `!` prefix), no enabled gshadow password, explicit members, or administrators, and no other local account or group referring to the GID. An empty `/etc/group` password is not a trusted explicit lock value, and account-tool documentation differs on its semantics, so the tool fails closed. After the account disappears, repeated local-passwd and NSS checks must also show no same-name identity. Only if the group remains after `userdel --` does the tool run `groupdel --`, then verify that both account and group remain absent. It subsequently verifies that `/etc/subuid` and `/etc/subgid` contain no same-name assignment, but does not guess at and rewrite a residual range. Any account-database state that cannot be established reliably returns nonzero and retains the deletion-recovery row. An old or non-sequential row whose account is already absent does not prove which same-name group may be deleted, so it too remains for operator handling.
...
`linux-temp-admin cleanup-expired` removes stale registry rows and orphaned sudoers files, sshd exceptions, and revoke tasks. It **never deletes an account**. Use `linux-temp-admin revoke` to delete an account and `linux-temp-admin status` to list them.

### Recover a missing identity sequence

If `linux-temp-admin doctor` explicitly reports that an otherwise valid v5 registry is missing `identity-sequence`, invites and registry mutations fail closed. This genuinely absent object is the only state eligible for the dedicated recovery command. If an object exists but is corrupt, has unsafe ownership or permissions, is a symlink, or carries a high-water mark below a registered UID, the command never overwrites it. Restore trusted state or preserve and investigate the host instead; do not delete or hand-edit the object to bypass the check.

Establish `N` independently from trusted history as the highest UID/GID the tool has **ever reserved**, including accounts that were later deleted and numbers burned by failed creations. The current passwd/group databases and surviving registry rows provide only a lower bound; they cannot prove the historical maximum. Do not guess or treat the largest surviving UID as complete history. After establishing that evidence, run this from a real interactive terminal:

```bash
/usr/bin/sudo /usr/local/sbin/linux-temp-admin recover-identity-sequence --highest N
```

The command displays the highest registered UID, current local allocation range, and observed floor, and refuses a value below those observations. It warns when the value reaches or exceeds the allocation maximum and would leave later invites exhausted. Continuing requires typing the exact `RECOVER IDENTITY-SEQUENCE HIGHEST=N` phrase it prints. Only after confirmation does the process acquire the global lifecycle lock, reread registry and local UID/GID state, reject any intervening change, and publish a root:root mode `0600` sequence with no-replace semantics. The reported `safe-after` starts an identity-isolation window of at least 65 seconds; until it passes, the next generated-name invite does not skip the synchronous deferred-job drain. Success and in-lock failures are recorded as `registry.identity-sequence.recover` audit events. Run `linux-temp-admin doctor` again afterward and confirm that the integrity error is gone.
