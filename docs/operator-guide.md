...
不传 `--port` 时，程序先读取 `sshd -T` 的有效端口，再回退到静态 `sshd_config` 或 OpenSSH 默认端口 22。只要有效配置探测失败、配置文件损坏或两个来源不能给出一致的可靠结论，邀请就会失败并要求显式传入 `--port`；`doctor` 会同时显示暂定值和原始诊断，不会把回退值报告为已确认端口。
...
删除授权、身份及删除前的任务/进程静默检查通过后，程序会在受控的 mail/Home 清理和 `userdel` 前持久化删除恢复见证；首次 Home 清理后、`userdel` 前仍会再次复核任务、进程和稳定身份（用户名、UID/GID、Home、第五字段世代见证；旧单标记账号则复核完整快照），随后重新确认同一 passwd 身份并再次按 UID/GID 清理可能在最终静默检查期间重建的 Home。若账号删除、删除后的 mail spool 复扫或任务清理中断，`status` 和 `doctor` 会显示删除恢复状态，同名 `invite` 会拒绝覆盖该见证，`cleanup-expired --compact` 也不会删除见证。账号已经不存在或仍精确匹配登记世代时，运行 `linux-temp-admin revoke --user <名>` 可继续恢复；账号缺失时，每轮窄范围邮件清扫前后都会复核本地 passwd 与 NSS 均无同名身份。上述两种状态下仍保留可识别的自动任务，但只有 systemd 任务会按重启策略自动重试，`at` 和旧的一次性任务需要人工重试。旧版、未登记或 pending 回滚只保留 UID 见证，若账号仍存在，必须人工核查后在交互终端运行 `linux-temp-admin revoke --user <名> --force` 并输入完整用户名，任何非交互调用都会被拒绝；这类活账号的旧自动任务会被当作孤儿任务取消，登记见证则保留供人工恢复。

对登记了 `SequentialID` 的账号，该位同时证明创建时 UID 与私有 GID 使用同一个预留号码。终删会在清理 Home/mail 前要求同名私有组仍存在、使用该 GID、`/etc/group` 密码字段明确禁用（`x`、`*` 或 `!` 前缀）、gshadow 没有可用组密码、显式成员或管理员，并且没有其他本地账号或组引用该 GID；空的 `/etc/group` 密码字段不是受信任的显式锁定值，而且不同账号工具对其语义说明并不一致，因此程序失败关闭。账号消失后还会通过本地 passwd 与 NSS 反复确认没有同名身份。`userdel --` 后组若仍存在，才执行 `groupdel --` 并再次确认账号与组均保持消失。程序随后检查 `/etc/subuid` 和 `/etc/subgid` 中没有同名分配，但不会猜测并改写残留范围。任一数据库状态无法可靠确认都会返回非零并保留删除恢复登记；账号已经消失的旧版或非顺序记录没有足够证据授权删除同名组，也会保留供人工处理。
