...
默认会安排在 24 小时后自动撤销。正常 systemd 主机上的手动或到期撤销会立即删除 sudo/sshd 授权、禁用登录、终止进程并清理个人任务，然后在前台返回；passwd 中的用户名和 UID/GID 会继续隔离至少一个任务轮询周期，由持久化 timer 在后台复查并完成账号、Home 和 mail 删除，避免为了安全而让终端等待 65 秒。systemd 不可用时仍同步等待并完成同样的安全清场。若账号已在程序外消失且没有删除恢复见证，程序会清理可安全识别的按用户名授权和任务，但只有在确认没有同名组或 subuid/subgid 残留后才移除登记；它不会猜测删除这些残留或 Home/mail。已经授权的撤销若提前持久化了恢复见证，则允许继续核对 UID 属主的传统单文件 mail spool 清扫；仅当 `SequentialID` 证明了原私有 GID 时，才可清理对应的残留私有组。subuid/subgid 只会验证无残留，Home 绝不会按缺失账号的旧路径恢复性删除。若安全检查或清理失败，程序会返回非零，保留账号（若仍存在）和登记，并尽力禁用仍存在的账号，供 systemd 重试或人工处理。即使启用了自动撤销，用完后也应立即手动撤销。
