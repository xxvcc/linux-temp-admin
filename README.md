...
`install` 复制的是**当前正在运行的那个二进制**。所以它只在你运行的是别处的副本时才有意义，比如 `sudo ./linux-temp-admin install`——路径前面的 `./` 是关键。如果你运行的就是已安装的 `/usr/local/sbin/linux-temp-admin`，它会拿自己覆盖自己，字节完全相同，属于空操作（加不加 `--force` 都一样）。
